...
export default function BrandLogo({ variant = 'shield' }: { variant?: 'shield' | 'monogram' | 'seal' | 'facebook' }) {
  ...
  // 4. FACEBOOK PROFILE (Avataras) - Naudok šitą profilio nuotraukai.
  // Tai sukurs solidų mėlyną kvadratą su baltu skydu. Tobula mažam formatui.
  if (variant === 'facebook') {
    return (
      <div className="w-64 h-64 bg-blue-700 flex items-center justify-center shadow-2xl">
        <div className="relative w-32 h-32 flex items-center justify-center">
          <svg viewBox="0 0 24 24" fill="none" className="w-full h-full text-white" xmlns="http://www.w3.org/2000/svg">
            <path d="M12 22C12 22 20 18 20 12V5L12 2L4 5V12C4 18 12 22 12 22Z" fill="currentColor"/>
          </svg>
          <svg viewBox="0 0 24 24" fill="none" className="w-16 h-16 absolute text-blue-700" xmlns="http://www.w3.org/2000/svg">
            <path d="M20 6L9 17L4 12" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
      </div>
